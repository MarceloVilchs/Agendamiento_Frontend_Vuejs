<template>
    <div>
      ...
      <div v-for="cita in citas" :key="cita.id" class="cita-container">
        ...
        <div class="cita-info">
          ...
          <div class="info-group"><strong>Nombre Paciente</strong><br />{{ cita.last_name }}</div>
          <div class="info-group"><strong>Nombre Paciente</strong><br />{{ cita.last_name2 }}</div>
  ...
  export default {
    ...
    methods: {
      // Método para obtener las citas del servidor
      fetchCitas() {
    axios
      .get(process.env.VUE_APP_API + "/usuarioFormulario/GET")
      .then((response) => {
